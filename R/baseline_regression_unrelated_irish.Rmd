...
```{r}
set.seed(123)
X_irish_geno_centered <- X_irish_geno - matrix(rep(colMeans(X_irish_geno), 
                                             each = nrow(X_irish_geno)), 
                                         nrow = nrow(X_irish_geno), 
                                         byrow = FALSE)
X_irish_cov_centered <- X_irish_cov - matrix(rep(colMeans(X_irish_cov), 
                                             each = nrow(X_irish_cov)), 
                                         nrow = nrow(X_irish_cov), 
                                         byrow = FALSE)
...
```{r}
set.seed(123)
n_samples <- nrow(X_irish_cov_centered)
irish_test_indices <- sample(1:n_samples, size = round(0.1 * n_samples), replace = FALSE)

# Split the data into training and testing sets
X_train_cov <- X_irish_cov_centered[-irish_test_indices, ]
X_test_cov  <- X_irish_cov_centered[irish_test_indices, ]

X_train_geno <- X_irish_geno_centered[-irish_test_indices, ]
X_test_geno  <- X_irish_geno_centered[irish_test_indices, ]
